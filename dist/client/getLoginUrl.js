...
exports.getOAuth2LoginUrl = getOAuth2LoginUrl;
exports.getLoginPageUrl = getLoginPageUrl;
exports.getAuthServerLoginUrl = getAuthServerLoginUrl;
exports.getAvailableOAuth2Providers = getAvailableOAuth2Providers;
exports.getOAuth2ProviderRedirectUrl = getOAuth2ProviderRedirectUrl;
/**
 ...
 */
async function getOAuth2LoginUrl(config, options) {
 ...
 */
function getLoginPageUrl(config) {
 ...
 */
function getAuthServerLoginUrl(config, options) {
    const base = config.authServerBaseUrl.replace(/\/$/, '');
 ...
 */
async function getAvailableOAuth2Providers(config) {
    const base = config.authServerBaseUrl.replace(/\/$/, '');
    ...
    return data.providers
        .filter((p) => p.enabled !== false && typeof p.loginUrl === 'string')
        .map((p) => ({
        provider: p.provider ?? '',
 ...
 */
function getOAuth2ProviderRedirectUrl(config, options) {
    const base = config.authServerBaseUrl.replace(/\/$/, '');
    const params = new URLSearchParams();
    if (options.redirectUri)
        params.set('redirect_uri', options.redirectUri);
